...
'Quartic_Neuron': {                 #Single neuron membrane modeled with quartic feedback and input.
   'DCF_values': ['Poly', 4, 1],
   'rhs_function': lambda t, y, p: [
       -p[0]*y[0]**4 + p[1]*y[1],
       -p[2]*y[1] + p[3]*np.tanh(y[0])
   ],
   'parameters_and_IC': [
       ([1.0, 0.5, 0.1, 1.0], [0.2, 0.0], 'feedback_quartic_neuron')
   ]
},
